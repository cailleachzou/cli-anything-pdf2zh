"""Install / uninstall the Xiaomi MiMo translator in the bundled pdf2zh EXE.
...
``patch/__init__.py::MIMO_TRANSLATOR_SOURCE`` so the source of truth
...
def is_installed(paths: Dict[str, str]) -> bool:
    ...
    return MIMO_CLASS_MARKER in text and MIMO_NAME_MARKER in text
...
def install(paths: Dict[str, str], *, backup: bool = True) -> Dict[str, Any]:
    """Append the MiMoTranslator class to the bundled translator.py and
    ...
    if not tr_text.endswith("\n"):
        tr_text += "\n"
    tr_text += MIMO_TRANSLATOR_SOURCE
    tr_path.write_text(tr_text, encoding="utf-8")

    # 2. Patch pdf2zh.py: add "MiMoTranslator," to the import list AND
    ...
    # 3. Patch converter.py: add "MiMoTranslator," to the import list
...
def _patch_converter_imports(text: str) -> tuple[str, int]:
    """Insert ``MiMoTranslator,`` in the import block of converter.py.
    ...
    def add_mimo(m: re.Match) -> str:
        head, body, tail = m.group(1), m.group(2), m.group(3)
        if "MiMoTranslator" in body:
            return m.group(0)
        new_body = body + "    MiMoTranslator,\n"
        return head + new_body + tail

    new_text, count = pattern.subn(add_mimo, text)
    return new_text, count


def _patch_converter_lookup(text: str) -> tuple[str, int]:
    """Insert ``MiMoTranslator,`` in the
    ...
    new_text, count = pattern.subn(add_mimo, text, count=1)
    return new_text, count


def _patch_pdf2zh_imports(text: str) -> tuple[str, int]:
    """Insert ``MiMoTranslator,`` in the import block of ``yadt_main``.
...
def _patch_pdf2zh_registrations(text: str) -> tuple[str, int]:
    """Insert ``MiMoTranslator,`` in the ``yadt_main`` translator list
...
def uninstall(paths: Dict[str, str], *, restore_backup: bool = True) -> Dict[str, Any]:
    """Remove the Xiaomi MiMo translator from the bundle.
...
def _strip_mimo_class(text: str) -> str:
